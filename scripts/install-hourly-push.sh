...
echo "  This repo lives under ~/Documents, which macOS protects. A launchd job"
echo "  cannot read it until /bin/bash has Full Disk Access:"
echo "    System Settings > Privacy & Security > Full Disk Access > + > Cmd-Shift-G"
echo "    > /bin/bash > Open, then toggle it on."
echo "  Verify with: launchctl kickstart gui/$(id -u)/$LABEL"
echo "  A run logging 'Operation not permitted' means the grant is still missing."
echo "  (Moving the repo outside ~/Documents avoids the grant entirely.)"
